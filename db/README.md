...
- Retención: mismo sweep de 72 h que los rasters (Worker `nexrad-l3-ops`, por `valid_time`); la reconciliación R2↔D1 cubre `rasters` **y** `wind_grids`. `ops` no cambió con la migración a Docker: lee/escribe por SQL D1 + API R2, sin importarle quién escribió las filas.
