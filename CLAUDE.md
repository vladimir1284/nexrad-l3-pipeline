...
1. **Contenedor LDM** (Unidata Local Data Manager): `ldmd` se suscribe al IDD con `request NNEXRAD` filtrado por sitios/productos; `pqact` entrega cada producto al procesador (PIPE o FILE + watcher). Config en `ldm/` (`ldmd.conf`, `pqact.conf`, `registry.xml`).
2. **Procesador Python 3.12**: decodifica con MetPy (`Level3File`), grilla datos polares/raster → malla regular AEQD centrada en el radar (nearest neighbor, preserva valores calibrados), escribe COG con Rasterio (valores físicos escalados, CRS + geotransform embebidos, overviews internos), sube a R2. Productos no-raster y metadata → D1.
