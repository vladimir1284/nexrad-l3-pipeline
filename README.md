...
Pipeline de ingesta de productos NEXRAD Level III desde el espejo S3 público del feed de NSF Unidata (`unidata-nexrad-level3`, acceso anónimo), con generación de artefactos geoespaciales (Cloud-Optimized GeoTIFF) en Cloudflare R2 y metadatos/fenómenos en Cloudflare D1.
...
Es el hermano "cloud/demo" de **LAMULA-Ingest**: misma lógica de dominio NEXRAD (decodificación de bloques PDB / Symbology / Tabular, extracción de fenómenos), pero cambia la fuente (bucket S3 público de Unidata en vez de nbtcp desde un ORPG) y el destino (R2 + D1 en vez de FTP + PostgreSQL).

## Arquitectura

```
Radares NEXRAD → NOAA/Unidata → s3://unidata-nexrad-level3 (público, anónimo)
        │
...
1. **Servicio poller (`l3proc poll`).** Cada ~60 s lista en el bucket público las claves nuevas por sitio×producto y las deposita en el directorio de entrada con escritura atómica. Watermark persistido (los reinicios no re-descargan historia) y catch-up capeado. Sustituye al contenedor LDM del diseño original — el IDD exige registro con Unidata; el bucket es el mismo feed sin registro. La decodificación es independiente del transporte: un LDM con `pqact` escribiendo `FILE` al mismo directorio encajaría sin tocar el resto.
2. **Procesador Python 3.12 (`l3proc watch`).** Servicio persistente con watcher (inotify/watchdog): decodifica el producto Level III con **MetPy** (`Level3File`), grilla los datos a una malla regular en proyección **AEQD centrada en el radar** (resampleo *nearest neighbor* para preservar valores calibrados), escribe el **COG calibrado** con Rasterio (niveles + scale/offset embebidos, CRS + geotransform, overviews internos) y lo sube a R2. Los fenómenos (Symbology/Tabular) y VWP requieren parsing propio sobre los bloques que MetPy expone crudos; esos productos no-raster y la metadata de cada raster van a D1. Procesados se borran; fallidos quedan en `failed/`.
...
- **Polling del bucket S3 público, no LDM/IDD** (revisada 2026-07-06). El IDD exige registro por host; el bucket da el mismo feed anónimo con 1–5 min de latencia, irrelevante frente al umbral de frescura de 30 min. La capa de decodificación se mantiene independiente del transporte.
...
- **Entrega poller → procesador: FILE + watcher.** El poller escribe el producto crudo a disco (tmp + rename, atómico) y un servicio Python persistente lo consume (inotify/watchdog). El fichero en disco da tolerancia a fallos, reintento y replay de crudos durante desarrollo, y es el contrato que cualquier transporte alternativo sabe cumplir. Los procesados se borran tras subir a R2/D1; los fallidos quedan para reproceso.
...
- **Un solo directorio de entrada para todos los sitios×productos.** El procesador no discrimina por nombre de fichero: decodifica el contenido. La selección de qué baja vive en la config del poller (`NEXRAD_SITES`/`NEXRAD_PRODUCTS` o flags).
...
- [NSF Unidata LDM](https://www.unidata.ucar.edu/software/ldm/) · [IDD feedtypes](https://www.unidata.ucar.edu/projects/idd/) — transporte alternativo si algún día hay registro IDD.
- [MetPy](https://unidata.github.io/MetPy/) — `metpy.io.Level3File` para decodificación NEXRAD Level III.
- Bucket S3 público `unidata-nexrad-level3` (claves `SITE_MNEMO_YYYY_MM_DD_HH_MM_SS`, sitio sin prefijo K/T) — fuente del poller y muestras reales para desarrollo, tests y replay.
