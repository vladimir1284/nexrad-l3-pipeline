...
def test_publish_vwp_idempotente():
    ...
    assert n == n2 == 13
    rows = pg.execute("SELECT * FROM vwp ORDER BY height_ft")
    assert len(rows) == 13
    assert rows[0]["height_ft"] == 400
    assert rows[0]["wind_dir_deg"] == 91.0
    assert rows[0]["wind_speed_kt"] == 8.0
    assert pg.execute("SELECT kind FROM products WHERE code = 48")[0]["kind"] == "vwp"
    assert pg.execute("SELECT site_id FROM radars")[0]["site_id"] == "AMX"
