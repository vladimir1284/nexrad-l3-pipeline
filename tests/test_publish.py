"""Publisher contra un Postgres falso respaldado por SQLite real.

Ejecutar los statements reales de ingest/storage/publish.py contra un
motor real (no un mock) valida su sintaxis y el schema de
db/pg_migrations/ a la vez — ver conftest.sqlite_compatible_pg_schema.
"""
...
def test_publish_sube_y_registra(pipeline_amx):
    ...
    producto = pg.execute("SELECT * FROM products")[0]
    assert producto["code"] == 153
    assert producto["mnemonic"] == "N0B"
    assert producto["kind"] == "raster"

    raster = pg.execute("SELECT * FROM rasters")[0]
...
def test_publish_es_idempotente(pipeline_amx):
    ...
    publish_cog(cog, prod, grid, r2, pg)
    first_seen = pg.execute("SELECT first_seen_at FROM radars")[0]["first_seen_at"]
    publish_cog(cog, prod, grid, r2, pg)

    assert len(pg.execute("SELECT * FROM rasters")) == 1
    assert len(pg.execute("SELECT * FROM radars")) == 1
    assert len(pg.execute("SELECT * FROM products")) == 1
    # first_seen_at se conserva; last_seen_at avanza con cada publicación.
    assert pg.execute("SELECT first_seen_at FROM radars")[0]["first_seen_at"] == first_seen
